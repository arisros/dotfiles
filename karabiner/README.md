...
¹ tmux's default `l` is last-window, but `tmux.conf:41` rebinds it to
...
(`@minimal-tmux-indicator-str`, `tmux.conf:74`). The browser layer had no such
cue, which made a two-key sequence feel like a guess. Now it drives two:

- **sketchybar** — `sketchybar/items/tmux_prefix.sh` adds a hidden item that
...
Both are driven by `__scripts__/karabiner-prefix-indicator.sh`, called from
...
must never cost you a keypress. Karabiner cannot expand a repo-relative path, so the rule file hardcodes
